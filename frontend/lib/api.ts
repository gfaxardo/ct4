...
export interface Claims14dResponse {
  status: string
  count: number
  total: number
  filters: Record<string, any>
...
export interface Claims14dSummaryRow {
  pay_week_start_monday: string
  milestone_value: number
  expected_amount_sum: number
  paid_confirmed_amount_sum: number
  paid_enriched_amount_sum: number
  pending_active_amount_sum: number
  pending_expired_amount_sum: number
  expected_count: number
  paid_confirmed_count: number
  paid_enriched_count: number
  pending_active_count: number
  pending_expired_count: number
}

export interface Claims14dSummaryResponse {
  status: string
  count: number
  filters: Record<string, any>
  rows: Claims14dSummaryRow[]
}

export async function getClaims14d(params?: {
  week_start?: string
  milestone_value?: number
  ...
  limit?: number
  offset?: number
}): Promise<Claims14dResponse> {
  const searchParams = new URLSearchParams()
  if (params) {
    if (params.week_start) searchParams.append('week_start', params.week_start)
    if (params.milestone_value) searchParams.append('milestone_value', String(params.milestone_value))
    if (params.paid_status) searchParams.append('paid_status', params.paid_status)
    if (params.window_status) searchParams.append('window_status', params.window_status)
    if (params.limit) searchParams.append('limit', String(params.limit))
    if (params.offset) searchParams.append('offset', String(params.offset))
  }
  const queryString = searchParams.toString()
  const res = await fetch(`${API_URL}/api/v1/yango/payments/claims_14d${queryString ? `?${queryString}` : ''}`)
  if (!res.ok) throw new Error('Error obteniendo claims 14d')
  return res.json()
}

export async function getClaims14dSummary(params?: {
  week_start?: string
  milestone_value?: number
  paid_status?: 'paid_confirmed' | 'paid_enriched' | 'pending_active' | 'pending_expired'
  window_status?: 'active' | 'expired'
  limit?: number
}): Promise<Claims14dSummaryResponse> {
  const searchParams = new URLSearchParams()
  if (params) {
    if (params.week_start) searchParams.append('week_start', params.week_start)
    if (params.milestone_value) searchParams.append('milestone_value', String(params.milestone_value))
  ...
  const queryString = searchParams.toString()
  const res = await fetch(`${API_URL}/api/v1/yango/payments/claims_14d/summary${queryString ? `?${queryString}` : ''}`)
  if (!res.ok) throw new Error('Error obteniendo resumen de claims 14d')
  return res.json()
}

// Claims Cabinet APIs (Nueva vista v_claims_payment_status_cabinet)
export interface ClaimsCabinetRow {
  driver_id?: string | null
  person_key?: string | null
  milestone_value: number
  lead_date: string
  ...
  payment_identity_status?: string | null
  payment_match_rule?: string | null
  payment_match_confidence?: string | null
  payment_status: 'paid' | 'not_paid'
  payment_reason: 'payment_found' | 'no_payment_found'
}

export interface ClaimsCabinetResponse {
  status: string
  count: number
  total: number
  filters: Record<string, any>
  rows: ClaimsCabinetRow[]
}

export interface ClaimsCabinetSummaryRow {
  pay_week_start_monday: string
  milestone_value: number
  expected_amount_sum: number
  paid_amount_sum: number
  not_paid_amount_sum: number
  expected_count: number
  paid_count: number
  not_paid_count: number
}

export interface ClaimsCabinetSummaryResponse {
  status: string
  count: number
  filters: Record<string, any>
  rows: ClaimsCabinetSummaryRow[]
}

export async function getClaimsCabinet(params?: {
  week_start?: string
  milestone_value?: number
  payment_status?: 'paid' | 'not_paid'
  limit?: number
  offset?: number
}): Promise<ClaimsCabinetResponse> {
  const searchParams = new URLSearchParams()
  if (params) {
    if (params.week_start) searchParams.append('week_start', params.week_start)
    if (params.milestone_value) searchParams.append('milestone_value', String(params.milestone_value))
  ...
  const queryString = searchParams.toString()
  const res = await fetch(`${API_URL}/api/v1/yango/payments/claims_cabinet${queryString ? `?${queryString}` : ''}`)
  if (!res.ok) throw new Error('Error obteniendo claims cabinet')
  return res.json()
}

export async function getClaimsCabinetSummary(params?: {
  week_start?: string
  milestone_value?: number
  payment_status?: 'paid' | 'not_paid'
  limit?: number
}): Promise<ClaimsCabinetSummaryResponse> {
  const searchParams = new URLSearchParams()
  if (params) {
    if (params.week_start) searchParams.append('week_start', params.week_start)
    if (params.milestone_value) searchParams.append('milestone_value', String(params.milestone_value))
  ...
  const queryString = searchParams.toString()
  const res = await fetch(`${API_URL}/api/v1/yango/payments/claims_cabinet/summary${queryString ? `?${queryString}` : ''}`)
  if (!res.ok) throw new Error('Error obteniendo resumen de claims cabinet')
  return res.json()
}
