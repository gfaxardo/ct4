...
import { getYangoReconciliationSummary, getYangoReconciliationItems, YangoReconciliationSummaryRow, YangoReconciliationItemRow } from '@/lib/api'
...
export default function YangoDashboard({
  ...
  async function loadItemsForWeek() {
    ...
    try {
      ...
      // #region agent log
      fetch('http://127.0.0.1:7243/ingest/baceb9d4-bf74-4f4f-b924-f2a8877afe92',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'YangoDashboard.tsx:loadItemsForWeek:response',message:'Items loaded from API (paginated)',data:{totalItems:allItems.length,weekFilter,chunks:Math.ceil(allItems.length/chunkLimit),firstItem:allItems[0]?{reconciliation_status:allItems[0].reconciliation_status,expected_amount:allItems[0].expected_amount,paid_is_paid:allItems[0].paid_is_paid}:null,anomalyItemsCount:allItems.filter(i=>isAnomaly(i)).length},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix',hypothesisId:'B,C'})}).catch(()=>{});
      // #endregion
  ...
  const weeklyData = summary.reduce((acc, row) => {
    ...
    // Usar amount_expected_sum de summary_ui si existe, sino sum_amount_expected
    ...
    // Si no, y el status es 'paid', usar el expected_sum de esa fila
    const paidSum = row.amount_paid_sum ?? (row.reconciliation_status === 'paid' ? expectedSum : 0)
    
    acc[week].amount_expected_sum += expectedSum
    acc[week].amount_paid_sum += paidSum
    
    // Usar count_expected de summary_ui si existe, sino count_expected o count_items
  ...
  if (hasWeekFilter && items.length > 0) {
    ...
    const realAnomalies = weekItems.filter(item => isAnomaly(item)).length
  ...
  const realAnomaliesCount = useMemo(() => {
    ...
    const anomalyItems = items.filter(item => isAnomaly(item))
    const count = anomalyItems.length
    // #region agent log
    fetch('http://127.0.0.1:7243/ingest/baceb9d4-bf74-4f4f-b924-f2a8877afe92',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'YangoDashboard.tsx:realAnomaliesCount:result',message:'realAnomaliesCount calculated',data:{totalItems:items.length,anomalyCount:count,sampleItem:items[0]?{reconciliation_status:items[0].reconciliation_status,expected_amount:items[0].expected_amount,paid_is_paid:items[0].paid_is_paid,paid_payment_key:items[0].paid_payment_key}:null,firstAnomaly:anomalyItems[0]?{reconciliation_status:anomalyItems[0].reconciliation_status,reason:computeAnomalyReason(anomalyItems[0]).code}:null},timestamp:Date.now(),sessionId:'debug-session',runId:'initial',hypothesisId:'D,F'})}).catch(()=>{});
    // #endregion
    return count
  }, [hasWeekFilter, items])
          ...
          <div className="px-6 py-4">
            <div className="space-y-3">
              {topReasons.map((reason, idx) => {
                const totalAnomalies = items.filter(item => isAnomaly(item)).length
