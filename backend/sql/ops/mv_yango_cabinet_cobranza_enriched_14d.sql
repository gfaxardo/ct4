-- ============================================================================
-- Vista Materializada: ops.mv_yango_cabinet_cobranza_enriched_14d
-- ============================================================================
-- PROPÓSITO:
-- MV final que materializa EXACTAMENTE lo que usa la tabla de Cobranza Yango,
-- incluyendo campos de scout attribution canónica. NUNCA joinear pesado en runtime.
-- ============================================================================
-- ESTRUCTURA:
-- - Base: ops.v_cabinet_financial_14d (campos financieros)
-- - Enriquecimiento: ops.v_scout_attribution + ops.v_dim_scouts (campos scout)
-- - Grano: 1 fila por driver_id
-- ============================================================================
-- REFRESH:
-- Ejecutar: REFRESH MATERIALIZED VIEW CONCURRENTLY ops.mv_yango_cabinet_cobranza_enriched_14d;
-- Se recomienda refrescar diariamente o después de actualizaciones de claims/scout
-- ============================================================================
...
-- Índices para optimizar filtros y queries
-- NOTA: El índice único es necesario para REFRESH CONCURRENTLY
...
'MV final que materializa Cobranza Yango con atribución scout canónica. Incluye todos los campos financieros de v_cabinet_financial_14d + campos de scout desde v_scout_attribution (multifuente) + nombres desde v_dim_scouts. Grano: 1 fila por driver_id. Se debe refrescar periódicamente (diariamente recomendado).';

COMMENT ON COLUMN ops.mv_yango_cabinet_cobranza_enriched_14d.scout_id IS 
'Scout ID canónico asignado al driver. Fuente: ops.v_scout_attribution (agrega múltiples fuentes con prioridad).';

COMMENT ON COLUMN ops.mv_yango_cabinet_cobranza_enriched_14d.scout_name IS 
'Nombre del scout desde module_ct_scouts_list (raw_name).';

COMMENT ON COLUMN ops.mv_yango_cabinet_cobranza_enriched_14d.scout_quality_bucket IS 
...
COMMENT ON COLUMN ops.mv_yango_cabinet_cobranza_enriched_14d.scout_source_table IS 
'Tabla fuente de donde proviene el scout_id (para auditoría).';

COMMENT ON COLUMN ops.mv_yango_cabinet_cobranza_enriched_14d.scout_priority IS 
