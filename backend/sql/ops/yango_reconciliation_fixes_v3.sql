-- ============================================================================
-- Fixes v3 para Conciliación de Pagos Yango
-- ============================================================================
-- Este script corrige:
-- 1. Naming: crea alias VIEW ops.yango_payment_ledger
-- 2. Corrige pay_week_start_monday NULL (debe ser NUNCA NULL)
-- 3. Define sort_date y pay_week_start_monday correctamente
...
-- ============================================================================
-- Cambios críticos:
-- - sort_date = COALESCE(payable_date, paid_date, paid_snapshot_at::date)
-- - pay_week_start_monday = date_trunc('week', sort_date)::date (NUNCA NULL)
-- - Mantener todas las columnas existentes (no romper contratos)
-- ============================================================================
...
unified_data AS (
    ...
    SELECT 
        NULL::date AS pay_week_start_monday,  -- Se calculará después
        pay_iso_year_week,
        payable_date,
        achieved_date,
        lead_date,
...
data_with_sort_date AS (
    -- Calcular sort_date y pay_week_start_monday
    SELECT 
        *,
        -- sort_date = COALESCE(payable_date, paid_date, paid_snapshot_at::date)
        COALESCE(
...
SELECT 
    ...
    -- count_drivers: distinct driver_id, si es null usar paid_raw_driver_name
    COUNT(DISTINCT 
        COALESCE(
            driver_id::text,
            paid_raw_driver_name
...
'Vista detalle UI-friendly de items de reconciliación Yango. Proporciona columnas limpias para tabla del frontend: pay_week_start_monday, sort_date, paid_raw_driver_name, milestone_value, expected_amount, paid_is_paid, reconciliation_status, paid_match_rule, paid_match_confidence, driver_id, person_key.';
...
'Fecha de ordenamiento: COALESCE(payable_date, paid_date, paid_snapshot_at::date).';

-- ============================================================================
-- VALIDACIONES
-- ============================================================================

-- Validación 1: Count de ledger (tabla vs alias)
-- SELECT 
...
-- Validación 3: Top 50 ordenado por pay_week_start_monday desc, sort_date desc
-- SELECT 
--     pay_week_start_monday,
--     sort_date,
--     payable_date,
--     paid_date,
--     paid_snapshot_at::date AS paid_snapshot_date,
--     reconciliation_status,
--     expected_amount,
--     paid_is_paid,
--     paid_raw_driver_name,
...
-- Validación 4: Conteo por reconciliation_status
-- SELECT 
--     reconciliation_status,
...
-- Validación 5: Conteo semanal en summary_ui
-- SELECT 
--     pay_week_start_monday,
--     milestone_value,
--     reconciliation_status,
...
-- Validación 6: Verificar que sort_date siempre existe
-- SELECT 
--     COUNT(*) AS total_rows,
--     COUNT(sort_date) AS rows_with_sort_date,
--     COUNT(*) - COUNT(sort_date) AS rows_without_sort_date,
--     CASE 
--         WHEN COUNT(*) = COUNT(sort_date) THEN 'OK: All rows have sort_date'
--         ELSE 'ERROR: Some rows missing sort_date'
--     END AS validation_status
-- FROM ops.v_yango_reconciliation_detail;
