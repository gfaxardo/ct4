-- ============================================================================
-- Fixes v4 para Conciliación de Pagos Yango - DEFINITIVO
-- ============================================================================
-- Este script corrige definitivamente:
-- 1. Alias VIEW ops.yango_payment_ledger (idempotente)
-- 2. Corrige ops.v_yango_reconciliation_detail: pay_week_start_monday NUNCA NULL
-- 3. Crea vistas UI-friendly (summary_ui, items_ui)
--
-- IMPORTANTE: Ejecutar este script una vez en DB. Luego correr ingest con
-- ops.ingest_yango_payments_snapshot() o vía endpoint.
-- ============================================================================

-- ============================================================================
-- 1. ALIAS VIEW: ops.yango_payment_ledger
-- ============================================================================
...
'Alias VIEW para ops.yango_payment_status_ledger. Mantiene compatibilidad con código legacy que referencia ops.yango_payment_ledger.';

-- ============================================================================
-- 2. CORREGIR ops.v_yango_reconciliation_detail
-- ============================================================================
-- Cambios críticos:
-- - sort_date = COALESCE(payable_date, paid_date, achieved_date, lead_date, created_at_export::date, paid_snapshot_at::date) NUNCA NULL
-- - pay_week_start_monday = COALESCE(existing, date_trunc('week', sort_date)::date) NUNCA NULL
-- - Mantiene todas las columnas existentes (no rompe contratos)
-- ============================================================================
...
unified_data AS (
    ...
    SELECT 
        NULL::date AS pay_week_start_monday,  -- Se calculará después desde sort_date
        pay_iso_year_week,
        payable_date,
        achieved_date,
        lead_date,
...
data_with_sort_date AS (
    -- Calcular sort_date con múltiples fallbacks (NUNCA NULL)
    SELECT 
        *,
        -- sort_date con fallbacks: payable_date, paid_date, achieved_date, lead_date, created_at_export::date, paid_snapshot_at::date
        COALESCE(
            payable_date,
            paid_date,
            achieved_date,
            lead_date,
            created_at_export::date,
            paid_snapshot_at::date
        ) AS sort_date
    FROM unified_data
)
-- SELECT final con pay_week_start_monday calculado (NUNCA NULL)
SELECT 
    -- pay_week_start_monday: usar el existente si hay, sino calcular desde sort_date
    COALESCE(
        pay_week_start_monday,
        date_trunc('week', sort_date)::date
    ) AS pay_week_start_monday,
    pay_iso_year_week,
    payable_date,
    achieved_date,
    lead_date,
    ...
    reconciliation_status,
    sort_date
FROM data_with_sort_date;

COMMENT ON VIEW ops.v_yango_reconciliation_detail IS 
'Vista detalle de reconciliación Yango que compara expected (ops.v_yango_receivable_payable_detail) vs paid (ops.v_yango_payments_ledger_latest). Matching preferido por driver_id+milestone_value, fallback por person_key+milestone_value. Status: paid (expected existe y paid=true), pending (expected existe pero no paid o paid=false), anomaly_paid_without_expected (paid=true pero no expected). pay_week_start_monday NUNCA es NULL (se calcula desde sort_date si no existe). sort_date siempre tiene valor con fallbacks múltiples.';

COMMENT ON COLUMN ops.v_yango_reconciliation_detail.sort_date IS 
'Fecha de ordenamiento: COALESCE(payable_date, paid_date, achieved_date, lead_date, created_at_export::date, paid_snapshot_at::date). Usada para calcular pay_week_start_monday si no existe. NUNCA es NULL.';

COMMENT ON COLUMN ops.v_yango_reconciliation_detail.pay_week_start_monday IS 
'Lunes de la semana de pago. NUNCA es NULL: usa el valor de expected si existe, sino calcula desde sort_date usando date_trunc(''week'', sort_date)::date.';

-- ============================================================================
-- 3. VISTA UI-FRIENDLY: Resumen Agregado Semanal
-- ============================================================================
...
SELECT 
    ...
    -- amount_paid_sum: suma de expected_amount donde paid_is_paid = true OR paid_payment_key IS NOT NULL
    SUM(expected_amount) FILTER (
        WHERE (paid_is_paid = true OR paid_payment_key IS NOT NULL) 
    ...
    -- count_expected: expected_amount IS NOT NULL
    COUNT(*) FILTER (WHERE expected_amount IS NOT NULL) AS count_expected,
    -- count_paid: paid_is_paid = true OR paid_payment_key IS NOT NULL
    COUNT(*) FILTER (WHERE paid_is_paid = true OR paid_payment_key IS NOT NULL) AS count_paid,
    -- count_drivers: distinct COALESCE(driver_id::text, paid_raw_driver_name)
    COUNT(DISTINCT 
        COALESCE(
            driver_id::text,
            paid_raw_driver_name
...
'Vista agregada semanal UI-friendly de reconciliación Yango. Agrupa por semana (pay_week_start_monday), milestone_value y reconciliation_status. Proporciona métricas limpias para UI: rows_count, amount_expected_sum, amount_paid_sum, amount_diff, count_expected, count_paid, count_drivers.';
...
'Suma de montos expected donde paid_is_paid = true OR paid_payment_key IS NOT NULL (montos que fueron pagados).';
...
'Vista detalle UI-friendly de items de reconciliación Yango. Proporciona columnas limpias para tabla del frontend: pay_week_start_monday, sort_date, paid_raw_driver_name, milestone_value, expected_amount, paid_is_paid, reconciliation_status, paid_match_rule, paid_match_confidence, driver_id, person_key, paid_payment_key, paid_snapshot_at.';
...
COMMENT ON COLUMN ops.v_yango_reconciliation_items_ui.sort_date IS 
'Fecha de ordenamiento: COALESCE(payable_date, paid_date, achieved_date, lead_date, created_at_export::date, paid_snapshot_at::date).';

-- ============================================================================
-- VALIDACIONES (Comentadas - descomentar para ejecutar)
-- ============================================================================

-- Validación 1: Counts ledger table vs alias
-- SELECT 
...
--     CASE 
--         WHEN (SELECT COUNT(*) FROM ops.yango_payment_status_ledger) = 
--              (SELECT COUNT(*) FROM ops.yango_payment_ledger) 
--         THEN 'OK: Counts match'
--         ELSE 'ERROR: Counts do not match'
--     END AS validation_status;

-- Validación 2: null_weeks debe ser 0
-- SELECT 
--     COUNT(*) AS null_weeks,
--     CASE 
--         WHEN COUNT(*) = 0 THEN 'OK: No NULL weeks'
--         ELSE 'ERROR: Found NULL weeks'
--     END AS validation_status
-- FROM ops.v_yango_reconciliation_detail
-- WHERE pay_week_start_monday IS NULL;

-- Validación 3: SELECT * summary_ui limit 20
-- SELECT 
--     pay_week_start_monday,
--     milestone_value,
--     reconciliation_status,
...
-- Validación 4: SELECT * items_ui limit 20
-- SELECT 
--     pay_week_start_monday,
--     sort_date,
--     paid_raw_driver_name,
